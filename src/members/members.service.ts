...
export class MembersService {
  ...
  async detail(a: Access, uid: string): Promise<Member> {
    a.requireStaff();
    identifier(uid);
    const member = await this.store.get<Member>(a.path('members', uid));
    if (!member?.active) throw new ApiError(404, 'not_found', 'Member not found.');
    return member;
  }
  async assign(a: Access, body: AssignMemberDto, manager: boolean): Promise<Member> {
    ...
    const requestedUnitId = manager ? '' : field(body, 'unitId', 120, true);
    const unitId = requestedUnitId ? identifier(requestedUnitId) : '';
    ...
    return this.store.transaction(async (tx) => {
      ...
      const unit = unitId ? await tx.get<Unit>(a.path('units', unitId)) : null;
      if (profile.houseId)
      ...
      if (unitId && (!unit || unit.occupied || unit.residentUid)) conflict('Choose a vacant unit.');
      const member: Member = {
        id: target.uid,
        uid: target.uid,
        name,
      ...
      this.audit.log(
        tx,
        a,
        manager ? 'Manager assigned' : unit ? 'Resident assigned' : 'Resident added',
        {
  ...
  async edit(a: Access, uid: string, body: EditMemberDto): Promise<Member> {
    ...
    return this.store.transaction(async (tx) => {
      ...
      const unit = member.unitId ? await tx.get<Unit>(a.path('units', member.unitId)) : null;
      if (member.unitId && (!unit || unit.residentUid !== uid))
        conflict('The resident unit assignment is inconsistent.');
  ...
  async remove(a: Access, uid: string, body: RemoveMemberDto): Promise<{ removed: boolean }> {
    ...
    return this.store.transaction(async (tx) => {
      ...
      const house = (await tx.get<House>(`houses/${a.houseId}`))!;
      const unit =
        member.role === 'renter' && member.unitId
          ? await tx.get<Unit>(a.path('units', member.unitId))
          : null;
      if (member.role === 'renter' && member.unitId && (!unit || unit.residentUid !== uid))
        conflict('The resident unit assignment is inconsistent.');
      ...
      tx.set(`houses/${a.houseId}`, {
        ...house,
        ...(member.role === 'renter'
          ? { residentCount: Math.max(0, house.residentCount - 1) }
          : { managerUid: '' }),
      });
      this.audit.log(
        tx,
        a,
        member.role === 'renter' ? 'Resident access removed' : 'Manager access removed',
        {
          subjectUid: uid,
          before: member,
          reason,
